...

{
    ...
    //  Программа запрашивает у пользователя готовность сыграть в "Угадай число"
    
    cout << "Привет! ";
    for (askint = 0; askint < 1;)
    {
        ...
        else
        {
            ...
            if (answer == "N" || answer == "n" || answer == "нет" || answer == "Нет")
            {
                ...
                //  В дальнейшем необходимо расширить масштаб проверки вводимых пользователем данных и добавить функционал возврата по меткам вместо завершения программы
            {
                system("cls");
...
int game(void)
{
    ...
    for (count1 = 1; count1 <= 3; count1++)
    {
        ...
        /*
            Если randominteger = strtoint тогда выдать сообщение о победе пользователя и !!!!!ПРЕДЛОЖИТЬ ПОВТОРИТЬ ИГРУ!!!!!;
